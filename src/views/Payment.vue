  ...
  <div class="box" @click.capture="boxBlock">
    <div class="wrap">
      ...
      <div class="buyMediums">
        <div class="mediumsTop" v-if="paintsFlag">
          <p class="mediumsTit">购买配套画材包</p>
          <p class="editBtn" @click="editBtn" v-if="editVal">管理</p>
          <p class="completeBtn" @click="completeBtn" v-if="completeVal">完成</p>
        </div>
        <div class="mediumsBox" v-if="paintsFlag">
          <div class="mediumsDiv" v-for="(item,index) in paymentArray.paints" :key="index">
            <div class="nameTop">
              <!-- <p
                class="choosenameIcon"
                @click="nameMethod(item,index)"
                :class="nameIndex==index?'nameActive':''"
                v-if="completeVal"
              ></p>-->
              <p
                class="choosenameIcon"
                @click="nameMethod(item,index)"
                :class="{nameActive:paintingArr.includes(index)}"
                v-if="completeVal"
              ></p>
              <!-- <p class="choosenameIcon" :class="nameIndex==index?'nameActive':''"></p> -->
              <p class="mediumsName">{{item.name}}</p>
            </div>
            <p class="mediumsPrice">
              <span class="mediumsLeft">
                <span class="priceSum">¥{{item.price}}</span>
                <span class="priceNum" v-if="editVal">x{{item.count}}</span>
                <span class="sumDiv" v-if="completeVal">
                  <span @click="cutSumBtn(index)">-</span>
                  <!-- <span class="editSum">1</span> -->
                  <input
                    v-model="item.count"
                    @keyup.enter="search"
                    @input="search($event,item)"
                    class="editSum"
                    type="number"
                  />
                  <span @click="addSumBtn(index)">+</span>
                </span>
              </span>
            </p>
          </div>
        ...
        <!-- 优惠券 -->
        <div class="couponDiv" @click="couponBtn" v-if="showNoFlag">
          <p>优惠券</p>
          <p class="rightDiv">
            <span v-if="couponBoxFlag1">选择优惠券</span>
            <span v-if="couponBoxFlag">{{couponPrice}}</span>
            <img class="rightBtn" src="../../static/moreRight.png" alt />
          </p>
        </div>
...
export default {
  ...
  methods: {
    search(event, item) {
      // console.log(event.currentTarget.value, item);
      let str = "" + item.count;
      if (str.indexOf(".") !== -1) {
        let arr = str.split("");
      ...
      if (item.count >= item.start_count) {
        // console.log("xiao11");
      } else {
        item.count = item.start_count;
        Toast({
          message: "不能低于购买限制",
          duration: 2000,
        });
